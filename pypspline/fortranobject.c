...
static void dump_attrs(const PyArrayObject* arr) {
  int rank = arr->nd;
...
#else
	/* Since ISCONTIGUOUS(obj), C_ARRAY is not too strong. */
	if (intent & F2PY_INTENT_INOUT) {
	  obj2 = NA_IoArray(obj, tAny, NUM_C_ARRAY);
	} else if (intent & F2PY_INTENT_IN) {
	  obj2 = NA_InputArray(obj, tAny, NUM_C_ARRAY);
	} else {
	  obj2 = NA_OutputArray(obj, tAny, NUM_C_ARRAY);
	}
#endif
	return obj2;
      }
...
/*       Py_INCREF(arr); */
  ...
  if (intent & F2PY_INTENT_INOUT) {
    ARR_IS_NULL(1,"intent(inout) argument must be an array.\n");
  }
